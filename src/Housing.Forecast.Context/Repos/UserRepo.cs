...
namespace Housing.Forecast.Context.Repos
{
    public class UserRepo : IRepo<User>
    {
        ...
        {
            var result = AsyncDBCall(u => u.Created.Date <= datetime.Date && (u.Deleted.Value == null || u.Deleted.Value.Date > datetime.Date));
            return result.Result;
        }
        ...
        /// </returns>
        private async Task<IEnumerable<User>> AsyncDBCall(Func<User, bool> lamdba) {
            var task = await Task.Factory.StartNew(() => {
                return _context.Users.Where(lamdba).ToList();
            });
            return task;
        }
    }
